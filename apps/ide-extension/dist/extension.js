...
async function checkServer() {
    try {
        await axios_1.default.get(`${API_BASE_URL}/health`, { timeout: 2000 });
        return true;
    }
    catch (error) {
...
function activate(context) {
    ...
    let optimizeUserCmd = vscode.commands.registerCommand('promptoptima.optimizeUserPrompt', async () => {
        ...
        vscode.window.withProgress({
            ...
            try {
                const response = await axios_1.default.post(`${API_BASE_URL}/optimize/user`, {
                    text,
                    run_judge: true
                });
                ...
                outputChannel.show();
                const action = await vscode.window.showInformationMessage('Prompt optimized successfully. See output channel.', 'Replace Selection');
    ...
    let auditSystemCmd = vscode.commands.registerCommand('promptoptima.auditSystemPrompt', async () => {
        ...
        vscode.window.withProgress({
            ...
            try {
                const response = await axios_1.default.post(`${API_BASE_URL}/optimize/system`, {
                    text,
                    run_judge: true
                });
                const data = response.data;
                ...
                if (data.data.conflicts_found?.length > 0) {
                    outputChannel.appendLine('\n\n=== CONFLICTS FOUND ===');
                    data.data.conflicts_found.forEach((c) => outputChannel.appendLine(`- ${c}`));
                }
                if (data.data.suggestions?.length > 0) {
                    outputChannel.appendLine('\n\n=== SUGGESTIONS ===');
                    data.data.suggestions.forEach((s) => outputChannel.appendLine(`- ${s}`));
                }
                outputChannel.show();
            }
            catch (error) {
    ...
    let scanCodeCmd = vscode.commands.registerCommand('promptoptima.scanCode', async () => {
        ...
        vscode.window.withProgress({
            ...
            try {
                const response = await axios_1.default.post(`${API_BASE_URL}/optimize/code`, {
                    text,
                    run_judge: false
                });
